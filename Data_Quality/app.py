...
# if 'dq_tool' not in st.session_state:
#     try:
#         st.session_state.dq_tool = DataQualityTool()
#     except Exception as e:
#         st.error("Data Quality Tool failed to initialize. Check console logs and configurations.")
#         st.exception(e)
#         st.stop()
# dq_tool_instance = st.session_state.dq_tool


# # ===================================================================
# # --- LOGO CODE START (THIS IS THE ONLY ADDITION) ---
# # ===================================================================
...
# st.sidebar.header("⚙️ Controls")
# entity_map = {"Tables": "table", "Views": "view", "Functions": "function", "Procedures": "procedure"}
# entity_display_opts = ["-- Select an Entity --"] + list(entity_map.keys())
# if "selected_entity_display" not in st.session_state:
#     st.session_state.selected_entity_display = entity_display_opts[0]

# def on_entity_change():
#     if 'comparison_results' in st.session_state:
#         del st.session_state['comparison_results']
# selected_entity_disp = st.sidebar.selectbox("1. Entity type:", entity_display_opts, key="selected_entity_display", on_change=on_entity_change)

# run_disabled = True
# if selected_entity_disp != '-- Select an Entity --':
#     entity_key = entity_map[selected_entity_disp]
#     st.sidebar.markdown("---")
#     mode_disp = st.sidebar.radio("2. Comparison mode:", ("Compare All Entities", "Compare Selected Entities"), key=f"rad_{entity_key}")
#     comp_mode = "all" if mode_disp == "Compare All Entities" else "selected"
#     sel_items = []
#     if comp_mode == "all":
#         run_disabled = False
...
#         avail_items = st.session_state.get(items_key, [])
#         if avail_items:
#             sel_items = st.sidebar.multiselect(f"3. Select {selected_entity_disp.lower()}:", avail_items, key=f"ms_{entity_key}")
#             run_disabled = not bool(sel_items)
#             if run_disabled:
#                 st.sidebar.warning(f"Select at least one {entity_key}.")
#         else:
...
# # --- Metric Bar and Summary Logic (This is your correct code and is unchanged) ---
# if 'comparison_results' in st.session_state and st.session_state.comparison_results:
#     results_list = st.session_state.comparison_results
...
# # --- Display Detailed Results Area (This is your correct code and is unchanged) ---
# st.subheader("📊 Comparison Details")
# if 'comparison_results' in st.session_state:
#     results_list = st.session_state.comparison_results
#     if not results_list:

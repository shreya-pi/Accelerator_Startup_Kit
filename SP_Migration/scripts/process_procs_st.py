...
from SP_Migration.scripts.py_test import run_single_test
...
class ProcessProcsPage:
    ...
    def display_page(self):
        ...
        with st.container(border=True):
            st.subheader("⚙️ Process Converted Scripts")
            st.markdown(
                """
            ...
            if st.button("🚀 **Process All Files**", width='stretch', type="primary"):
                try:
        ...
        with st.container(border=True):
            st.subheader("📊 Compare Original vs. Converted Scripts")
    ...
    def _load_file_for_editing(self):
        ...
        if rich_label and rich_label != "-- Select a procedure to compare --":
            # Extract the actual filename from the rich label (e.g., "my_proc.sql  [✅]" -> "my_proc.sql")
    ...
    def display_comparison_viewer(self):
        ...
        if not self.processed_dir.exists() or not self.extracted_dir.exists():
            st.warning(f"⚠️ Required directories not found."); return
        ...
        try:
            ...
            for filename in converted_files:
                ...
                status_icon = status_map.get(proc_name, "❔") # Default to '?' if no status found
                
                # # Format the label, e.g., "my_proc.sql  [✅]"
            ...
            st.selectbox(
                "Choose a procedure (Status: ✅=Success, ❌=Failed, ❔=Not Tested):", 
                options, 
            ...
            if st.session_state.editable_file_path:
                ...
                with col2:
                    ...
                    with btn_col2:
                        ...
                        if st.button("🧪 **Run Unit Test**", width='stretch', help="Run tests for this specific procedure."):
                    ...
                    if 'test_results' in st.session_state and st.session_state.test_results is not None:
                        ...
                        with st.container(border=True):
                            ...
                            with btn_hide_col:
                                ...
                                if st.button("❌ Hide", width='stretch', type="secondary", help="Clear the test results from view."):
                                    st.session_state.test_results = None
                                    st.rerun()
                            ...
                            metric_col2.metric("Passed ✅", successes)
                            metric_col3.metric("Failed ❌", failures, 
                            ...
                            for proc_name, test_type, status, reason, output in results:
                                if status == "✅ Success":
                                    with st.expander(f"✅ **{test_type}:** Passed", expanded=False):
                                        if output:
                                            st.text("Captured Output:")
                                            st.code(output, language='log')
                                else:
                                    with st.expander(f"❌ **{test_type}:** Failed", expanded=True):
                    ...
                    if st.session_state.edit_mode:
                        ...
                        if st.button("💾 **Save Changes**", width='stretch', type="primary"):
                            try:
                                ...
                                st.toast(f"✅ Saved changes to {file_to_save.name}", icon="💾")
            ...
            st.error("❌ An error occurred while setting up the comparison viewer:")
            st.exception(e)
    ...
    def run_single_procedure_test(self):
        ...
        if not processed_file_path.exists():
            st.error(f"❌ **Test Failed:** Could not find the processed file at `{processed_file_path}`. Please ensure you have run the 'Process All Files' step for this procedure.")
            return None
